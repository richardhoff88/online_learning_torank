...
def single_injection_UCB_attack(n_arms, target_arm, rho, T, means, std_devs, sigma = 1, delta = 0.05):
...
def experiment_synthetic_single_injection(T=int(1e4), n_arms=10, rho=1.0, sigma=1.0):
    means = np.random.rand(n_arms)
    std_devs = np.full(n_arms, sigma)
    target_arm = np.argmin(means)

    _, attack_trials_list, target_pulls, _, target_pull_ratios = single_injection_UCB_attack(
        n_arms, target_arm, rho, T, means, std_devs)

    plt.figure(figsize=(10, 6))
    plt.plot(range(1, T + n_arms), target_pull_ratios, label="Target Arm Selection Ratio")
    plt.xlabel("Rounds")
    plt.ylabel("Target Arm Selection Ratio")
    plt.title("Target Arm Selection Ratio Over Time (Single Injection Attack)")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    experiment_synthetic_single_injection()